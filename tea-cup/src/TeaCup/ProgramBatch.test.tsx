...
import { Cmd, Dispatcher, Maybe, noCmd, nothing, Result, Sub, Task } from 'tea-cup-core';
...
describe('program test batch', () => {
  ...
  async function doTest(strictMode: boolean, expectations: MvuExpectations) {
    ...
    const mounted = mount(strictMode ? <React.StrictMode>{p}</React.StrictMode> : p);
    ...
    await expect.poll(() => mounted.find('#foo')?.text(), { timeout: 2000, interval: 500 }).toEqual('m1m2m3m4');
